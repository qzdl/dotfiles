...
** testing
*** helpers
#+begin_src bash :tangle test-helpers.sh
headline() {
  echo "============================"
  echo $@
  echo "============================"
...
#+end_src
*** test-system
#+begin_src bash :tangle test-system.sh
...
#+end_src
*** update and test
#+begin_src bash :tangle update-dotfiles.sh
...
│   ├── gofmt.scm
│   ├── go-mvdan-sh.scm
│   └── python-flake8-black.scm
├── packages-python-3.6.4.scm
├── python-3.6.4.scm
├── qzdl
│   ├── device
│   │   └── donutrust.scm
...
**** test
#+begin_src bash
test-load-file .config/guix/qzdl/system/donutrust.scm
#+end_src
**** operating system
#+begin_src scheme
(operating-system
...
alias yt="youtube-dl --add-metadata -i -o '%(upload_date)s-%(title)s.%(ext)s'" \
      ...
      res='source ~/.bashrc' \
      xo='xdg-open' \
      wa='cat $HOME/.config/aliasrc'

# guix reconfigure, and debug to the repl
gRd ()  { sudo -E guix repl -L "$HOME/.config/guix/qzdl";}
gR ()  { sudo -E guix system -L "$HOME/.config/guix/qzdl" reconfigure "$HOME/.config/guix/qzdl/device/$(hostname).scm"; }
# guix profile commands; make it easy to switch and check
gse()  { gsp "$GUIX_EXTRA_PROFILES/$@"; }
gsp()  { export GUIX_PROFILE="$@"; . "$GUIX_PROFILE/etc/profile"; }
